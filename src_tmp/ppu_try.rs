...
impl Ppu {
  ...
  fn fill_bg_scanline(&mut self) {
    ...
    let scroll_x = self.scx % 8;

    for x in 1..20 {
      let tilemap_x = (x + self.scx/8) & 31;
      let tilemap_id = tilemap + 32 * (y/8) as u16 + tilemap_x as u16;
      ...
      for bit in 0..8 {
        let pixel_x = scroll_x + 8*x + bit;
        if pixel_x >= 160 { break; }
