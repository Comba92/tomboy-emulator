...
mod cpu_step_tests {
    ...
    use tomboy_emulator::cpu::{self, Cpu};
  ...
  impl CpuMock {
    fn from_cpu(cpu: &Cpu) -> Self {
      Self {
        pc: cpu.pc, sp: cpu.sp, a: cpu.a, b: cpu.bc.hi(), c: cpu.bc.lo(), 
        d: cpu.de.hi(), e: cpu.de.lo(), f: cpu.f.bits(), h: cpu.hl.hi(), l: cpu.hl.lo(),
        ram: Vec::new(),
      }
    }
  }
  ...
  fn cpu_from_mock(mock: &CpuMock) -> Cpu {
    ...
    cpu.bc.set_hi(mock.h);
    cpu.bc.set_lo(mock.l);
    cpu.sp = mock.sp;
    cpu.pc = mock.pc;
  ...
  #[test]
  fn cpu_test_one() {
    let json = include_str!("GameboyCPUTests/v2/01.json");
